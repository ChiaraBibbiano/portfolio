...
while (have_posts()) : the_post();
    ...
    $pres_title = get_field('pres_title');
    $pres_text = get_field('pres_text');
    $gallery_steps = get_field('gallery_steps');
    $dif_title = get_field('dif_title');
    $dif_text = get_field('dif_text');
    ...
    <article class="single-project">
        <div class="single-project__header">
            ...
            <?php if ($back_link && !empty($back_link['url'])) : ?>
                <a class="link" href="<?= esc_url($back_link['url']); ?>">
                    <?= esc_html($back_link['title']); ?> <span class="arrow" aria-hidden="true">↗</span>
                </a>
            <?php endif; ?>
        </div>
        <!--GALERIE 5 images -->
        ...
        <?php if ($site_link && !empty($site_link['url'])) : ?>
            <div class="single-project__site-link-wrap">
                <a class="single-project__site-link link" href="<?= esc_url($site_link['url']); ?>"
                   target="_blank">
                    <?= esc_html($site_link['title']); ?> <span class="arrow  " aria-hidden="true">↗</span>
                </a>
            </div>
        <?php endif; ?>
        <!--  PRÉSENTATION  -->
        <div class="single-project__press">
            <?php if ($pres_title) : ?>
        ...
        <div class="single-project__zigzag" aria-hidden="true">
            <svg class="zigzag__svg" viewBox="0 0 750 320" xmlns="http://www.w3.org/2000/svg"
                ...
                />
                <!-- Labels étapes -->
                <text x="25" y="60" class="zigzag__label" text-anchor="start">ANALYSE &amp;</text>
                <text x="25" y="75" class="zigzag__label" text-anchor="start">RECHERCHE</text>

                <text x="140" y="225" class="zigzag__label" text-anchor="start">MOODBOARD</text>

                <text x="330" y="90" class="zigzag__label" text-anchor="start">PROTOTYPE</text>
                <text x="360" y="105" class="zigzag__label" text-anchor="start">UX</text>

                <text x="458" y="295" class="zigzag__label" text-anchor="start">DESIGN</text>
                <text x="458" y="380" class="zigzag__label" text-anchor="start">UI</text>

                <text x="620" y="95" class="zigzag__label" text-anchor="start">DÉVELOPPMENT</text>

            </svg>
        </div>
        <!-- diff  -->
        <div class="single-project__diff">
            <?php if ($dif_title) : ?>
